<script setup lang="ts">
	...
	async function handleApprove(closeModal: () => void) {
		const success = await approve(props.log.id, adminRemarksText.value, internNotes)
	...
	<div>
		...
		<UModal v-if="isPending">
			...
			<template #content="{ close }">
				<UCard>
					<template #header>
						<h3 class="text-base font-semibold leading-6 text-gray-900 dark:text-white">{{ internName }}</h3>
					</template>
					...
					<template #footer>
						<div class="flex flex-col gap-4">
							...
							<div v-if="log.intern_notes">
								<p class="text-sm font-semibold text-gray-500 dark:text-gray-400">Intern's Notes</p>
								<p class="mt-1 w-full text-sm text-gray-700 italic dark:text-gray-300">"{{ log.intern_notes }}"</p>
							</div>

							<!-- Admin's remarks section -->
							<div>
								<p class="text-sm font-semibold text-gray-500 dark:text-gray-400">Admin Remarks (Optional)</p>
								<UTextarea
									v-model="adminRemarksText"
									:rows="4"
							...
							<div class="flex justify-center">
								<UButton label="Approve" color="primary" :loading="isApproving" @click="handleApprove(close)" />
							</div>
						</div>
					</template>
				</UCard>
		...
		<UCard v-else class="flex h-50 w-full flex-col" :ui="{ body: 'flex-1 overflow-y-auto' }">
			...
			<div v-if="log.intern_notes || log.admin_remarks" class="mt-4 space-y-3 border-t border-gray-200 pt-4 text-sm dark:border-gray-700">
				<!-- Display Intern's Notes -->
				<div v-if="log.intern_notes">
					<p class="font-medium text-gray-600 dark:text-gray-400">Intern's Notes:</p>
