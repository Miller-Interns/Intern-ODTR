...
import { getInternDetailsById, getTimeLogsByInternId } from '../service/intern-details.service'
export async function useGetInternDetails(db: Kysely<DB>, internId: string): Promise<InternDetailsResponse> {
	...
	const completedHoursRaw = timeLogsResult.filter((log) => log.status === true).reduce((sum, log) => sum + (log.total_hours || 0), 0)
