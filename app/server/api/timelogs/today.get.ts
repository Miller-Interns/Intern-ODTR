...
import { getPendingToday } from '~/server/use-case/time-logs/get-pending-logs-today'
...
export default defineEventHandler(async (event) => {
	...
	const completeLogs = fetchedLogs.filter(
		(log): log is Omit<DashboardLog, 'time_out'> & { time_out: string } => {
