	...
	try {
		...
		try {
			...
			throw createError({
				...
				data: (error as any).issues // Provide Zod issues for debugging
			});
		}

		// try {
		// 	const internId = event.context.params?.id as string;
		// 	const [intern, timelogs] = await exportInternTimelogs({ internId }, event.context);
		// 	const exportData: ExportTimeLogsResponse = {
		// 		intern: {
		// 			name: intern.user.name,
		// 			officialHours: '9:00 - 6:00 PM', // Or get from intern object
		// 		},
		// 		timelogs: timelogs,
		// 	};
