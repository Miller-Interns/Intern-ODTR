<template>
	<div class="flex min-h-screen flex-col bg-gray-50 pb-20">
		...
		>
			...
			<pre
				v-if="error.message"
				class="text-black-400 mt-2 text-xs"
				>{{ error.message }}</pre
			>
		</div>
		...
		<!-- Main Content -->
		<div
			v-else-if="dashboardData && typedUser"
			class="space-y-6 p-4"
		>
			<!-- Header -->
			<header>
				...
				<p
					v-if="!internStatus"
					class="pt-2 pb-3 text-amber-600 italic"
				>
					Your account is currently inactive.
				</p>
				<p
					v-else-if="activeTimeLog"
					class="text-black-500 pt-2 pb-3 italic"
				>
					Your time log is in progress
				</p>
				<p
					v-else-if="lastCompletedLog"
					class="text-black-500 pt-2 pb-3 italic"
				>
			...
			<div v-if="!internStatus">
				<UCard class="shadow-lg">
					<div class="text-center">
						<p class="mb-4">Your account is currently inactive. You cannot time in until an administrator reactivates your account.</p>
					</div>
				</UCard>
			</div>
				...
				<UCard class="mb-6 shadow-lg">
					...
					>
						...
						<div class="flex items-center rounded-lg bg-blue-50 p-3 text-blue-800">
							...
							/>
							<p class="text-xs text-black">To time out with less than 8 hours, please add a reason in the note field.</p>
						</div>
					</div>
					<div
						v-else
				...
				<UCard class="shadow-lg">
					<template #header>
						<div class="flex items-center justify-between">
							...
							<UBadge
								v-if="lastCompletedLog.status === true"
								color="success"
								variant="soft"
							>
								Approved
			...
			<div>
				...
				<UCard class="shadow-lg">
					<div class="space-y-1 text-center">
						...
						<UProgress
							v-model="renderedHours"
							:max="totalHours"
							color="success"
						/>
						<p class="text-black-800 text-lg font-semibold">{{ renderedHours.toFixed(0) }} / {{ totalHours }} Hours</p>
					</div>
				</UCard>
			</div>
		</div>
...
<script setup lang="ts">
	...
	// FIX: Define the refs for the UProgress component directly in the page.
	const renderedHours = ref(0)
	const totalHours = ref(0)

	// Pass these refs into our composable.
	const {
		pending,
		error,
		dashboardData,
		...
		isTimeOutDisabled,
		isSubmitting,
		progressPercentage, // We get the computed value back
		handleTimeOut,
	} = useDashboard(renderedHours, totalHours) // Pass them in here
</script>
