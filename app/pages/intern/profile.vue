			...
			<UCard class="shadow-lg">
				<div class="flex items-start gap-4">
					<div class="relative w-max">
						<UAvatar
							:src="data.profile.intern_picture || undefined"
							alt="Avatar"
							class="h-16 w-16"
						/>
						<button
						...
						>
							...
							<UIcon
								...
								class="animate-spin text-sm text-white"
							/>
						</button>
						<input
							ref="fileInput"
					...
					<div>
						...
						<UBadge
							v-if="data.profile.status === Status.ONGOING"
							color="success"
							variant="soft"
							class="mt-1"
						>
						...
						<UBadge
							v-else-if="data.profile.status === Status.COMPLETED"
							color="secondary"
							variant="soft"
							class="mt-1"
						>
						...
						<p class="text-black-500 text-sm">Hours: {{ data.profile.hours_completed }} / {{ data.profile.required_hours }}</p>
					</div>
				</div>
			</UCard>

			<div class="relative py-2">
				<div class="relative flex justify-center"><span class="bg-black-50 text-black-500 px-2 text-sm">Personal Info</span></div>
			...
			>
				<section>
					...
					<div class="pb-5">
						...
						<h2 class="text-black-800 text-left font-semibold">{{ data.profile.emergency_contact_person }}</h2>
					</div>
					<div>
						<h1 class="text-black-500 text-sm">Emergency Contact Number</h1>
						<h2 class="text-black-800 text-left font-semibold">{{ data.profile.emergency_contact_number }}</h2>
					</div>
				</section>
				<section>
					...
					<div class="pb-5">
						...
						<h2 class="text-black-800 text-left font-semibold">{{ data.profile.course }} - {{ data.profile.year }}</h2>
					</div>
					<div class="pb-5">
			...
			>
				...
				>
					...
					<section class="space-y-4">
						...
						>
							<UInput
								v-model="formState.emergency_contact_person"
								class="w-full"
					...
					<section class="mt-6 space-y-4">
						...
						>
							<UInput
								v-model.number="formState.required_hours"
								type="number"
								class="w-full"
...
<script setup lang="ts">
	...
	// Create the ref for the file input here in the component, as it's tied to the template.
	const fileInput = ref<HTMLInputElement | null>(null)

	// All the complex logic is now neatly contained within our new composable.
	// We pass the fileInput ref into the composable so it can control it.
	const {
		data,
		pending,
		error,
