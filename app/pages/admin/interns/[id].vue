...
const { data, pending, error, refresh } = useFetch<InternDetailsResponse>(
	() => `/api/interns/${internId.value}`
)
...
<template>
	<div class="p-4 sm:p-6 lg:p-8">
		<div v-if="pending" class="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-3">
			<USkeleton v-for="i in 3" :key="i" class="h-60 w-full" />
		</div>

		<UAlert v-else-if="error || !form" icon="i-lucide-triangle-alert" color="error" variant="subtle"
			title="Error Loading Data" description="Could not find intern data." />

		<UForm v-else :state="form" @submit="saveChanges">
			<div class="mb-4 flex items-center justify-between">
				<UButton to="/admin/interns" icon="i-lucide-arrow-left" color="neutral" variant="ghost"
					aria-label="Back to interns list" />
			</div>

			<UCard class="mb-6">
				<InternProfileHeader :intern="form" :is-editing="isEditing" :preview-src="avatarPreviewUrl"
					@update:status="handleStatusUpdate" @upload-picture="handlePictureUpload" />
			</UCard>

			<UTabs :items="tabItems" variant="link" :ui="{ trigger: 'grow' }" class="w-full gap-4" color="neutral">
				<!-- Personal Info Tab -->
				...
				<template #timelog>
					...
					<UButton label="Export DTR" color="primary" variant="solid" icon="i-heroicons-arrow-down-tray" block
						class="mt-6" />
				</template>
			</UTabs>
		</UForm>
