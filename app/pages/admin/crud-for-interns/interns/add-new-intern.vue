  ...
  <UContainer class="py-8">
    <div class="flex items-center gap-4 mb-6">
      <UButton
        ...
        class="-ml-4"
        aria-label="Back"
        :to="batchId ? `/admin/crud-for-interns/batches/${batchId}` : '/batches'"
      />
...
const { data: schoolsData, pending: isSchoolsLoading } = await useFetch<SchoolResponse[]>('/api/crud-for-interns/school', { lazy: true })
...
async function handleFormSubmit(event: FormSubmitEvent<AddInternDTO>) {
  ...
  try {
    ...
    await $fetch('/api/crud-for-interns/interns', {
      method: 'POST',
      body: submissionPayload,
    })

    toast.add({ title: 'Success!', description: 'Intern has been added successfully.', color: 'success' })
    await router.push(`/admin/crud-for-interns/batches/${batchId.value}`)
