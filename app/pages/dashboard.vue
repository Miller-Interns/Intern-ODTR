  ...
  <div class="bg-gray-50 min-h-screen flex flex-col pb-20">
    ...
    <div v-else-if="dashboardData && typedUser" class="p-4 space-y-6">
      ...
      <div v-if="activeTimeLog">
        <UCard class="mb-6">
          ...
          <div v-if="!showRemarksInput" class="text-center">
            <UButton @click="showRemarksInput = true" variant="link" icon="i-heroicons-plus-circle" class="mb-2">Add
              Remarks
              (Optional)</UButton>
            <div v-if="elapsedTime < 8"
              class="flex items-center justify-center bg-blue-50 p-3 rounded-lg text-blue-800">
              <UIcon name="i-heroicons-information-circle" class="text-xl mr-2" />
              <p class="text-xs">To time out with less than 8 hours, please add a reason in the note field.</p>
            </div>
          </div>
          <div v-else>
        ...
        <UCard>
          <template #header>
            <div class="flex justify-between items-center">
              ...
              <UBadge color="primary" variant="soft">Pending Approval</UBadge>
            </div>
          </template>
          <div class="space-y-4">
      ...
      <UCard>
        ...
        <div class="bg-white rounded-2xl shadow p-5 space-y-3 text-center">
          <p class="text-sm font-medium text-gray-500">
            {{ progressPercentage.toFixed(0) }}%
          </p>

          <!-- Custom Static Progress Bar (no animation) -->
          <div class="w-full h-2 bg-gray-200 rounded-full overflow-hidden" role="progressbar"
            :aria-valuenow="Math.min(Math.max(progressPercentage, 0), 100)" aria-valuemin="0" aria-valuemax="100">
            <div class="h-full bg-green-500 rounded-full transition-none animate-none"
              :style="{ width: Math.min(Math.max(progressPercentage, 0), 100) + '%' }" />
          </div>

          <p class="font-semibold">
            {{ renderedHours.toFixed(2) }} / {{ totalHours }} Hours
          </p>
        </div>
      </UCard>
    </div>
...
// Watcher to sync data and manage the timer
...
// --- Computed Properties & Formatters ---
const isTimeOutDisabled = computed(() => {
  if (isSubmitting.value) { return true; }
  if (elapsedTime.value < 8) { return intern_notes.value.trim() === ''; }
  return false;
});
...
async function handleTimeOut() {
  ...
  try {
    ...
    // On success, show toast and refresh data to show the "submitted" view.
